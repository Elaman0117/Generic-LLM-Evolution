#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# VARIANT: dates board - X = releaseDate timestamp, linear, no mapping.
...
def compute_scores(data):
    ...
    # X = release timestamp (UTC seconds) from leaderboard metadata.
    n_miss = 0
    for m in valid:
    ...
    ts1 = max(float(m["x_value"]) for m in valid)
    d0 = _dt.datetime.fromtimestamp(ts0, tz=_dt.timezone.utc).strftime("%Y-%m-%d")
    d1 = _dt.datetime.fromtimestamp(ts1, tz=_dt.timezone.utc).strftime("%Y-%m-%d")
    print(f"  Date range: {d0} - {d1} ({len(valid)} dated)")
...
def compute_pareto(models):
    """Pareto frontier (max ability, earliest release) over the given model set.

    x_value = release timestamp; smaller (earlier) is better.  a dominates
    b iff ability>= and date<= with one strict.
    """
...
def build_axis_mapping(models, brand_frontiers):
    # Exponential mapping for RELEASE DATE - x = (a^(b*t)-1)/(a^b-1),
    # t = (ts-ts_min)/(xmax-ts_min), c = 0 fixed. (a,b) fitted on the
    # 11 brand-frontier chart-visible models (target = in-group rank
    # quantiles). Pins (0,0)/(1,1) exact; convex (a>1,b>0) spreads recent
    # dates. ts > xmax chart-excluded but table-kept (over_max).
    print("Fitting exp mapping x = (a^(b*t)-1)/(a^b-1) on 11 brand frontiers...")
    fmodels = [m for fr in brand_frontiers.values() for m in fr
               if m.get("x_value") is not None and m.get("chart_y") is not None]
    fts = sorted(float(m["x_value"]) for m in fmodels)
    xmax = fts[-1]
    xmin = min(float(m["x_value"]) for m in models if m.get("x_value") is not None)
    span = xmax - xmin
    d0 = _dt.datetime.fromtimestamp(xmin, tz=_dt.timezone.utc).strftime("%Y-%m-%d")
    d1 = _dt.datetime.fromtimestamp(xmax, tz=_dt.timezone.utc).strftime("%Y-%m-%d")
    print("  frontier set: %d models, date range %s - %s" % (len(fmodels), d0, d1))
    ...
    while i < n:
        j = i
        while j + 1 < n and fts[j + 1] == fts[i]:
            j += 1
        tq = (i + j) / 2.0 / max(n - 1, 1)
        for k in range(i, j + 1):
            targets[fts[k]] = tq
        i = j + 1
    tt = np.array([(v - xmin) / span for v in fts])
    qq = np.array([targets[v] for v in fts])
    bvec = np.linspace(0.2, 5.0, 250)
    best = None
    for aa in np.logspace(np.log10(1.1), np.log10(50.0), 250):
        den = np.power(aa, bvec) - 1.0
        num = np.power(aa, np.outer(bvec, tt)) - 1.0
        f = num / den[:, None]
        ok = np.all(np.isfinite(f), axis=1) & (f.min(axis=1) >= 0.0) & (f.max(axis=1) <= 1.0)
        mse = np.mean((f - qq[None, :]) ** 2, axis=1)
        mse = np.where(ok, mse, np.inf)
        k = int(np.argmin(mse))
        if best is None or float(mse[k]) < best[0]:
            best = (float(mse[k]), float(aa), float(bvec[k]))
    for _ in range(2):
        _, a0, b0 = best
        best = None
        for aa in np.logspace(np.log10(a0 / 1.5), np.log10(a0 * 1.5), 200):
            for bb in np.linspace(max(b0 * 0.7, 0.05), b0 * 1.3, 200):
                den1 = aa ** bb - 1.0
                f = (np.power(aa, bb * tt) - 1.0) / den1
                if not (np.all(np.isfinite(f)) and f.min() >= 0.0 and f.max() <= 1.0):
                    continue
                mse = float(np.mean((f - qq) ** 2))
                if best is None or mse < best[0]:
                    best = (mse, float(aa), float(bb))
    mse0, a_best, b_best = best
    f = (np.power(a_best, b_best * tt) - 1.0) / (a_best ** b_best - 1.0)
    maxdev = float(np.max(np.abs(f - qq)))
    print("  best a=%.4f b=%.4f mse=%.6f maxdev=%.4f" % (a_best, b_best, mse0, maxdev))
    tg = np.linspace(0.0, 1.0, 20001)
    gg = (np.power(a_best, b_best * tg) - 1.0) / (a_best ** b_best - 1.0)
    print("  positivity proof: min/max f on [0,1] = %.12f/%.12f" % (float(gg.min()), float(gg.max())))
    assert bool(np.all(np.isfinite(gg))) and float(gg.min()) >= 0.0 and float(gg.max()) <= 1.0
    denb = a_best ** b_best - 1.0
    inv = np.log(gg * denb + 1.0) / (b_best * math.log(a_best))
    err = float(np.max(np.abs(inv - tg)))
    print("  inverse roundtrip: max |t_inv(f(t))-t| = %.3e (must be < 1e-9)" % err)
    assert err < 1e-9, "inverse roundtrip violated"
    den1 = a_best ** b_best - 1.0
    def fmap(v):
        v = float(v)
        if v <= xmin:
            return 0.0
        if v >= xmax:
            return 1.0
        t = (v - xmin) / span
        return float((a_best ** (b_best * t) - 1.0) / den1)
    for m in withx:
        m["axis_x"] = fmap(m["x_value"])
        m["over_max"] = float(m["x_value"]) > xmax
    decades = []
    for y in range(_dt.datetime.fromtimestamp(xmin, tz=_dt.timezone.utc).year,
                   _dt.datetime.fromtimestamp(xmax, tz=_dt.timezone.utc).year + 1):
        ts = _dt.datetime(y, 1, 1, tzinfo=_dt.timezone.utc).timestamp()
        if xmin < ts <= xmax:
            decades.append({"price": ts, "x": fmap(ts)})
    ...
    if over_max:
        ...
        for _m in sorted(over_max, key=lambda u: -float(u["x_value"])):
            print("    date=%s model=%s" % (_m.get("x_datestr"), _m.get("model")))
    vis = [m for m in withx if m.get("chart_y") is not None and not m.get("over_max")]
    left = sum(1 for m in vis if float(m["axis_x"]) < 0.5)
    mapping_meta = {
        "mapping": "exp mapping x = (a^(b*t)-1)/(a^b-1), t normalized dates",
        "function": "x = (a^(b*t)-1)/(a^b-1) with a=%.4f b=%.4f c=0; f(ts_min)=0, f(xmax)=1 exactly; ts>xmax chart-excluded (table x=1.0)" % (a_best, b_best),
        "knots": 2,
        "fit": {
            "fitted_models": n,
            "fit_set": "11 brand-frontier models (chart-visible)",
            "method": "least-squares fit of (a,b) over grid (pins exact, c=0)",
            "a": a_best, "b": b_best, "c": 0.0,
            "mse_vs_quantile": mse0,
            "max_abs_deviation_vs_quantile": maxdev,
            "uniform_density": False,
        },
        "left_edge_cost": xmin,
        "left_edge_label": _fmt_x_tick(xmin),
        "free_models": 0,
        "over_max_models": len(over_max),
        "plotted_models": len(vis),
        "total_models": len(models),
        "min_positive_cost": xmin,
        "x_at_min_positive_cost": 0.0,
        "max_cost": xmax,
        "x_at_max_cost": 1.0,
...
def analyze_x_distribution(models, mapping_meta):
    ...
    result = {
        ...
        "note": ("exp mapping x = (a^(b*t)-1)/(a^b-1) on normalized dates; pins (0,0)/(1,1)"),
    }
...
def _fmt_x_tick(v):
    """timestamp -> 'YYYY-MM' (dates tick labels, UTC)."""
    return _dt.datetime.fromtimestamp(v, tz=_dt.timezone.utc).strftime("%Y-%m")
...
def plot_analysis(models, pareto, brand_frontiers, x_dist, mapping_meta):
    ...
    method = (
        f"X轴: x = (a^(b*t)-1)/(a^b-1) (a={_fit.get('a', 0):.2f}, b={_fit.get('b', 0):.3f}；取自 11品牌前沿) | Pareto = 能力高且时间早"
    )
...
def _gen_candidates(m, P, R, w, h, pi_j, polylines, rad, future=None):
    ...
    #    距离罚分 —— 按能力从低到高连接放置时，上方邻居稍后还需要同一段线的空间；
...
def _place_labels(ax, fig, plot_models, pareto, brand_frontiers, exclusions):
    ...
    # 的点按能力从低到高连接连接，消除 Qwen3.8 Max→2.4T A95B、MiniMax M2.1→M2 的
    ...
    placements = []
    # V16-B：能力升序（按能力从低到高连接）。降序会造成「入侧级联」——链条顶端
    ...
    # 但更低" 这类情形，V15 起一律视为违反；同时点堆叠退化为「上方的
...
def _draw_pareto_lines(ax, placements, pareto, brand_frontiers):
    ...
    # V12-C：顶点按 (axis_x, 综合能力升序) 排序，同时点按能力从低到高连接连接
    _vkey = lambda m: (float(m["axis_x"]), float(m["composite_ability"]))
    polylines = []
    ...
    output = {
        "metadata": {
            ...
            "methodology": (
                ...
                "Pareto = non-dominated by release date (earlier is better); "
                ...
                "X-axis: linear release-date mapping x = (ts-ts_min)/(xmax-ts_min) "
                "(xmax = 11 brand-frontier max); left edge = earliest date in DB; "
            ...
            "x_source": "AA leaderboard metadata (releaseDate)",
            "total_models": len(models),
        ...
        "all_models": [
            {
                ...
                "release_timestamp": (float(m["x_value"]) if m.get("x_value") is not None else None),
...
def generate_readme(models, pareto, brand_frontiers, x_dist, mapping_meta):
    ...
    lines = []
    lines.append("# LLM Leaderboard — 综合能力 vs 发布时间\n")
    ...
    lines.append("共收录 **Status: All**（含已弃用）的全部模型；按重新归一化后的综合能力排序。"
                 ...
                 f"综合能力 ≥ 该级的 {n_vis} 个模型入图，{ya.get('below_baseline', 0)} 个能力低于第一级的不出现在图中；发布时间大于品牌前沿最大值的模型同样不入图"
                 "（表列出全部模型）。\n")
    lines.append("| # | 品牌 | 模型 | 综合能力 | 发布时间 | 横轴位置 | 帕累托 |")
    lines.append("|---|------|------|---------|-----------|-----------|------|")
    ...
    for i, m in enumerate(ranked):
        ...
        rd = m.get("release_date") or m.get("x_datestr") or "—"
        lx = m.get("axis_x")
    ...
    lines.append("4. **Pareto前沿** = 不被任何其他模型支配的模型（综合能力 ≥ 且发布时间 ≤（更早），且至少一项严格更优）")
    ...
    if x_dist.get("plotted_models"):
        ...
        lines.append(f"\u6a2a\u8f74\u4e3a\u53d1\u5e03\u65f6\u95f4\u6307\u6570\u6620\u5c04\uff1ax = (a^(b*t)-1)/(a^b-1)\uff1bt \u4e3a\u53d1\u5e03\u65f6\u95f4\u6309\u6700\u65e9\u548c\u6700\u5927\u6620\u5c04\u5230 [0,1]"
                     f"\uff1ba = {fit.get('a', 0):.3f}\uff0cb = {fit.get('b', 0):.3f}\uff0c\u7528 11 \u54c1\u724c\u524d\u6cbf\u5b9a\u51fa\uff1bmse = {fit.get('mse_vs_quantile', 0):.6f}\u3002\n")
        lines.append("- **\u51fd\u6570\u7aef\u70b9**\uff1a\u6700\u65e9\u53d1\u5e03 \u2192 x = 0\uff1b\u524d\u6cbf\u6700\u5927\u53d1\u5e03 \u2192 x = 1\uff1b")
        ticknames = ", ".join(_fmt_x_tick(d["price"]) for d in dec_ticks)
        lines.append(f"- \u6a2a\u8f74\u65f6\u95f4\u6807\u7b7e\uff1a{ticknames}")
        lines.append(f"- \u4e2d\u4f4d\u6570\u4f4d\u7f6e {x_dist.get('median', 0):.3f}\uff1b\u5de6 {x_dist.get('left_half_models', 0)} \u4e2a\uff0c\u53f3 {x_dist.get('right_half_models', 0)} \u4e2a")
        md = _dt.datetime.fromtimestamp(mapping_meta.get('max_cost', 0), tz=_dt.timezone.utc).strftime('%Y-%m-%d')
        lines.append(f"- \u524d\u6cbf\u6700\u5927\u53d1\u5e03\u65f6\u95f4 {md} \u2192 x = 1\uff1b\u5927\u4e8e\u8be5\u503c\u7684\u6a21\u578b\u4e0d\u5165\u56fe\uff0c\u8868\u683c\u4e2d\u6709\u3002")
        lines.append("")
# ── 标注规则 ─────────────────────────────────────────────────────────
    lines.append("## 图中标注规则\n")
    ...
    lines.append("4. **标签位置与序列同向（V15）**：品牌前沿上越靠右上的模型，其标签重心必须同时更靠右且更靠上 —— "
                 ...
                 "同时点堆叠即：上方模型的标签既在上方、也不更左）；初始放置违反时就近重摆"
    ...
    lines.append("## X：发布时间\n")
    lines.append("**X = 发布时间（线性）**\n")
    lines.append("发布时间为 releaseDate；左端为最早发布时间。\n")
# ── 数据来源 ─────────────────────────────────────────────────────────
    lines.append("### 数据来源\n")
    ...
    lines.append(f"**模型数**: {len(ranked)}（总体帕累托前沿 {len(pareto)} 个；图表入图 {n_vis} 个）  ")
    ...
    lines.append("图表说明：**灰色实线** = 总体帕累托前沿；**彩色细线** = 十一个品牌的单独帕累托前沿"
                 ...
                 "连接，同时点按能力从低到高连接）；品牌前沿模型圆点同样使用品牌颜色。"
                 ...
                 f"能力低于该级的 {ya.get('below_baseline', 0)} 个模型和发布时间大于品牌前沿最大值的模型不出现在图中；"
                 "横轴为发布时间（线性）。\n")
...
def main():
    ...
    print("=" * 60)
    print("LLM Leaderboard Pareto Analysis — DATES variant (linear X, no cost pipeline)")
    ...
    withx = [m for m in models if m.get("x_value") is not None]
    print(f"  Models with dates: {len(withx)}/{len(models)}")
